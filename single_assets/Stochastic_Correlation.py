...
from SingleAssetPricingModels import stochastic_correlation_model
...
def lprior(theta):
    ...
    return np.sum(np.log(np.exp(-((theta - prior_mu) ** 2) / (2* prior_std ** 2) * 1e-10) / 
                         np.sqrt(2 * np.pi * prior_std) * 1e-10))
...
def llike(theta, xproc, nproc, r):
    ...
    denominator = (sigma**2 + nproc[1:-1] * delta**2)
    denominator = np.where(denominator < 1e-10, 1e-10, denominator)  # Avoid division by zero

    logprobX = (-0.5 * np.log(2 * np.pi * denominator) 
               - 0.5 * ((xproc[1:] - xproc[:-1] - ((r - lambd * k) + nproc[1:-1] * nu))**2 / denominator))
...
def lfcd_Nt(t, nproc, xproc, theta, r):
    ...
    denominator = (sigma**2 + nproc[t] * delta**2)
    if denominator < 1e-10:  # Avoid division by zero
        denominator = 1e-10  # Use a small value instead

    # p(N_t| dX_t)
    if t >= 0 and t < T:
        val += nproc[t] * np.log(lambd) - lambd - special.gammaln(nproc[0] + 1)

        val += (-0.5 * np.log(2 * np.pi * denominator)
                - 0.5 * ((xproc[t] - xproc[t-1] - ((r - lambd * k) + nproc[t] * nu))**2 / denominator))
...
def gibb_mh(sims, data, theta0, N0, theta_proposal_cov=None, r=0.1, burnin=None):
    ...
    for i in range(1, sims):
        ...
        for t in range(T + 1):
            ...
            # Check for invalid values (NaN or inf) before calculating laprob
            if np.isnan(log_can) or np.isnan(log_curr) or np.isinf(log_can) or np.isinf(log_curr):
                laprob = -np.inf  # Reject the sample if invalid
            else:
                laprob = log_can - log_curr  # Normal acceptance probability
        ...
        # Check for invalid values (NaN or inf) before calculating laprob
        if np.isnan(log_posterior_can) or np.isnan(log_posterior_old) or np.isinf(log_posterior_can) or np.isinf(log_posterior_old):
            laprob = -np.inf  # Reject if invalid
        else:
            laprob = log_posterior_can - log_posterior_old  # Normal acceptance probability
...
if __name__ == '__main__':
    ...
    # Generate predictions using the stochastic correlation model
    S0 = data.iloc[0]  # Initial asset price (starting price of the stock)
    v0 = 0.2  # Initial volatility (this can be chosen or computed)
    rho0 = 0.0  # Initial correlation
    kappa_rho = 1.0  # Mean reversion speed of correlation
    theta_rho = 0.0  # Long-term mean of correlation
    sigma_rho = 0.1  # Volatility of correlation
    kappa = 1.0  # Mean reversion speed of variance
    theta = 0.2  # Long-term mean of variance
    sigma = 0.1  # Volatility of variance
    steps = T  # Number of steps (same as the number of time periods)

    # y_preds, _, _ = stochastic_correlation_model(S0, v0, rho0, kappa_rho, theta_rho, sigma_rho, kappa, theta, sigma, 0.1, T, steps)
    """
    summary(out_theta, ["sigma", "lambda", "nu", "delta"])
    plots(out_theta, ["sigma", "lambda", "nu", "delta"])
    """
    # print(data.shape, y_preds.shape)
